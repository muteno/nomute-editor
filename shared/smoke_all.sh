...
set -u
cd "$(dirname "$0")/.."
L1=$(mktemp) L2=$(mktemp) L3=$(mktemp) L4=$(mktemp) L5=$(mktemp) L6=$(mktemp) L7=$(mktemp) L8=$(mktemp) L9=$(mktemp) L10=$(mktemp) L11=$(mktemp) L12=$(mktemp) L13=$(mktemp); R1=-1; R2=-1; R3=-1; R4=-1; R5=-1; R6=-1; R7=-1; R8=-1; R9=-1; R10=-1; R11=-1; R12=-1; R13=-1; PRE=0
...
rm -f "$L1" "$L2" "$L3" "$L4" "$L5" "$L6" "$L7" "$L8" "$L9" "$L10" "$L11" "$L12" "$L13"
if [ "$R1" -eq 0 ] && [ "$R2" -eq 0 ] && [ "$R3" -eq 0 ] && [ "$R4" -eq 0 ] && [ "$R5" -eq 0 ] && [ "$R6" -eq 0 ] && [ "$R7" -eq 0 ] && [ "$R8" -eq 0 ] && [ "$R9" -eq 0 ] && [ "$R10" -eq 0 ] && [ "$R11" -eq 0 ] && [ "$R12" -eq 0 ] && [ "$R13" -eq 0 ]; then echo "── smoke_all 전부 PASS"; exit 0; fi
echo "── smoke_all FAIL (geni=$R1 · preview=$R2 · winnav=$R3 · dlclip=$R4 · rank=$R5 · popup=$R6 · trend=$R7 · editdock=$R8 · parity=$R9 · sweep=$R10 · launch=$R11 · chan=$R12 · editprev=$R13)"; exit 1
