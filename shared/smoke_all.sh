...
set -u
cd "$(dirname "$0")/.."
L1=$(mktemp) L2=$(mktemp) L3=$(mktemp) L4=$(mktemp) L5=$(mktemp) L6=$(mktemp) L7=$(mktemp) L8=$(mktemp) L9=$(mktemp) L10=$(mktemp); R1=-1; R2=-1; R3=-1; R4=-1; R5=-1; R6=-1; R7=-1; R8=-1; R9=-1; R10=-1; PRE=0
...
node shared/smoke_parity.js > "$L9" 2>&1 & P9=$!   # 크로스-탭 미리보기 렌더 등가(코어 10종 · 포트대 8831~ · 260719 이식 사고 근본원인 기계화 — 편집↔AI 생성 나란히 대조)
node shared/smoke_sweep.js > "$L10" 2>&1 & P10=$!   # 정렬 계약 상비(픽토 4분할·소머리 좌변·팝업 R-라인 · 코어 8종 · 포트대 8836~ · 260720 Q256 70캡쳐 스윕발 승격 — 확정 계약 축만·휴리스틱 비채택)
...
rm -f "$L1" "$L2" "$L3" "$L4" "$L5" "$L6" "$L7" "$L8" "$L9" "$L10"
if [ "$R1" -eq 0 ] && [ "$R2" -eq 0 ] && [ "$R3" -eq 0 ] && [ "$R4" -eq 0 ] && [ "$R5" -eq 0 ] && [ "$R6" -eq 0 ] && [ "$R7" -eq 0 ] && [ "$R8" -eq 0 ] && [ "$R9" -eq 0 ] && [ "$R10" -eq 0 ]; then echo "── smoke_all 전부 PASS"; exit 0; fi
echo "── smoke_all FAIL (geni=$R1 · preview=$R2 · winnav=$R3 · dlclip=$R4 · rank=$R5 · popup=$R6 · trend=$R7 · editdock=$R8 · parity=$R9 · sweep=$R10)"; exit 1
