...
#   geni(13종)+preview(10종·코어)+winnav(코어 6종 · 260717 Q02) 병렬 실행 — 포트대 분리 설계(8791~/8796~/8801~)라 무충돌 ·
...
set -u
cd "$(dirname "$0")/.."
L1=$(mktemp) L2=$(mktemp) L3=$(mktemp); R1=-1; R2=-1; R3=-1; PRE=0
...
rm -f "$L1" "$L2" "$L3"
if [ "$R1" -eq 0 ] && [ "$R2" -eq 0 ] && [ "$R3" -eq 0 ]; then echo "── smoke_all 전부 PASS"; exit 0; fi
echo "── smoke_all FAIL (geni=$R1 · preview=$R2 · winnav=$R3)"; exit 1
