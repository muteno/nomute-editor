...
set -u
cd "$(dirname "$0")/.."
L1=$(mktemp) L2=$(mktemp) L3=$(mktemp) L4=$(mktemp) L5=$(mktemp) L6=$(mktemp) L7=$(mktemp) L8=$(mktemp) L9=$(mktemp) L10=$(mktemp) L11=$(mktemp) L12=$(mktemp) L13=$(mktemp) L14=$(mktemp) L15=$(mktemp) L16=$(mktemp); R1=-1; R2=-1; R3=-1; R4=-1; R5=-1; R6=-1; R7=-1; R8=-1; R9=-1; R10=-1; R11=-1; R12=-1; R13=-1; R14=-1; R15=-1; R16=-1; PRE=0
...
node shared/smoke_ghead.js > "$L16" 2>&1; R16=$?   # 트렌드 그룹 헤더·금융 행 4분할 수평선 잉크 실측(코어 5종 · 포트대 8881~ · 260723 Q471 승격 — 겹침·잠복 편심 실사고 기계화 · 직렬 꼬리)
...
echo "════ smoke_ghead (rc=$R16) ════"; cat "$L16"
rm -f "$L1" "$L2" "$L3" "$L4" "$L5" "$L6" "$L7" "$L8" "$L9" "$L10" "$L11" "$L12" "$L13" "$L14" "$L15" "$L16"
if [ "$R1" -eq 0 ] && [ "$R2" -eq 0 ] && [ "$R3" -eq 0 ] && [ "$R4" -eq 0 ] && [ "$R5" -eq 0 ] && [ "$R6" -eq 0 ] && [ "$R7" -eq 0 ] && [ "$R8" -eq 0 ] && [ "$R9" -eq 0 ] && [ "$R10" -eq 0 ] && [ "$R11" -eq 0 ] && [ "$R12" -eq 0 ] && [ "$R13" -eq 0 ] && [ "$R14" -eq 0 ] && [ "$R15" -eq 0 ] && [ "$R16" -eq 0 ]; then echo "── smoke_all 전부 PASS"; exit 0; fi
echo "── smoke_all FAIL (geni=$R1 · preview=$R2 · winnav=$R3 · dlclip=$R4 · rank=$R5 · popup=$R6 · trend=$R7 · editdock=$R8 · parity=$R9 · sweep=$R10 · launch=$R11 · chan=$R12 · editprev=$R13 · vidattach=$R14 · wip=$R15 · ghead=$R16)"; exit 1
