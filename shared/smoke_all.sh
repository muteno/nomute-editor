...
#   geni(13종)+preview(10종·코어)+winnav(코어 10종 · 260717 Q02)+dlclip(교차 twin 코어 3종 · 260717 Q06)+rank(정렬 코어 7종 · 260717 Q05)+popup(앵커 팝업 셸 SSOT 패리티 코어 5종 · 260717) 병렬 실행 — 포트대 분리 설계(8791~/8796~/8801~/8806~/8811~/8816~)라 무충돌 ·
...
set -u
cd "$(dirname "$0")/.."
L1=$(mktemp) L2=$(mktemp) L3=$(mktemp) L4=$(mktemp) L5=$(mktemp) L6=$(mktemp); R1=-1; R2=-1; R3=-1; R4=-1; R5=-1; R6=-1; PRE=0
...
rm -f "$L1" "$L2" "$L3" "$L4" "$L5" "$L6"
if [ "$R1" -eq 0 ] && [ "$R2" -eq 0 ] && [ "$R3" -eq 0 ] && [ "$R4" -eq 0 ] && [ "$R5" -eq 0 ] && [ "$R6" -eq 0 ]; then echo "── smoke_all 전부 PASS"; exit 0; fi
echo "── smoke_all FAIL (geni=$R1 · preview=$R2 · winnav=$R3 · dlclip=$R4 · rank=$R5 · popup=$R6)"; exit 1
