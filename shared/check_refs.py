...
_DESIGN_BASELINE = {
    ...
    'viewer/sb.html': {'accent_raw': 0, 'blur': 10, 'hex': 1, 'accent_hex': 0, 'green_wash': 0, 'legacy_green': 0},   # blur12→10 = 도크 유리 폐지 미러(운영자 260802 4차 — .topdock backdrop blur(--blur-l)+webkit −2 · 알파 .72→1 · thumb 정본 동반).   # 콘티(스토리보드) 뷰어 편입(260717 실측 seed — 평의회 8인 arch 발견: 260714 신설분 미등재 = nb와 동일 사각 재발). blur12 = k.html 바이트 계승 사본분(iobtn류 13px/14px saturate 글래스 + var(--blur-s) 토큰 blur도 세는 카운터 특성 · 신규 창작 0) · hex1 = :root 밖 잔존 1(k 계승). 여타 게이트(autocomplete·soremeori 등) 편입은 nb 관례대로 위반 선정리 후 후속.
