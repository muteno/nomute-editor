...
_DESIGN_BASELINE = {
    ...
    'viewer/edit.html': {'accent_raw': 0, 'blur': 8, 'hex': 6, 'accent_hex': 0, 'green_wash': 0, 'legacy_green': 0},   # 편집기 탭 신설 편입(신규 뷰어 게이트 사각 봉합 관례 · 260710). blur4 = .urlclip 글래스 backdrop+webkit 2 + 대기 스크림(.wscrim) blur(5px) webkit+표준 2 = conv와 동수·전부 계승(신규 창작 0). hex4 = 입력 bg #0e0f11×2(URL·구간) + vstage #000×2 = conv 관행 내. +blur4·hex2 = 자막 편집기 이식(배치 B-2 260711 — ly.html 원문 CSS 그대로 = 창작 0·ly에서 검증된 값 복사 계승: .code 배경 #0e0f11·pre 색 #eef7f0 + 편집기 글래스 blur).
...
def check_viewer_js():
    ...
    rc = 0
    for rel in ('viewer/index.html', 'viewer/thumb.html', 'viewer/ly.html', 'viewer/k.html', 'viewer/track.html', 'viewer/conv.html', 'viewer/edit.html'):
        try:
...
def check_tokens_link():
    ...
    rc = 0
    for rel in ('viewer/thumb.html', 'viewer/ly.html', 'viewer/k.html', 'viewer/track.html', 'viewer/conv.html', 'viewer/edit.html'):
        try:
            html = open(os.path.join(ROOT, rel), encoding='utf-8').read()
        except Exception:
            continue
...
def check_soremeori():
    ...
    for rel in ('viewer/thumb.html', 'viewer/k.html', 'viewer/ly.html', 'viewer/track.html', 'viewer/conv.html', 'viewer/edit.html'):
        try:
