...
def check_viewer_js():
    ...
    rc = 0
    for rel in ('viewer/index.html', 'viewer/thumb.html', 'viewer/ly.html', 'viewer/k.html', 'viewer/track.html', 'viewer/conv.html', 'viewer/edit.html', 'viewer/song.html', 'viewer/nb.html', 'viewer/sb.html'):   # nb·sb 편입(평의회 Q165 — 색 baseline엔 있는데 JS 구문 게이트만 누락이던 커버리지 드리프트 봉합 · 현행 rc=0 실측)
        try:
...
def check_icon_ssot():
    ...
    rc = 0
    for rel in ('viewer/index.html', 'viewer/thumb.html', 'viewer/ly.html', 'viewer/k.html', 'viewer/track.html', 'viewer/conv.html', 'viewer/edit.html', 'viewer/song.html', 'viewer/nb.html', 'viewer/sb.html'):   # nb·sb 편입(평의회 Q165 — 아이콘 SSOT 게이트 정합 · 현행 위반 0 실측)
        try:
            html = open(os.path.join(ROOT, rel), encoding='utf-8').read()
        except Exception:
            continue
