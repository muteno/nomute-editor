...
_DESIGN_BASELINE = {
    ...
    'viewer/conv.html': {'accent_raw': 0, 'blur': 6, 'hex': 4, 'accent_hex': 0, 'green_wash': 0, 'legacy_green': 0},   # 변환 탭 신설 편입(신규 뷰어 게이트 사각 봉합 관례 · 260710). blur6 = .urlclip 글래스 backdrop+webkit(track 계승) + 대기 스크림(.wscrim) blur(5px) 운영자 픽 webkit+표준 + 위치 미리보기 오버레이(.scrub) blur(8px) webkit+표준(track .scrub 계승). hex = 팔레트 :root 제외 잔여(track 관행 내).
...
def check_viewer_js():
    ...
    rc = 0
    for rel in ('viewer/index.html', 'viewer/thumb.html', 'viewer/ly.html', 'viewer/k.html', 'viewer/track.html', 'viewer/conv.html'):
        try:
...
def check_tokens_link():
    ...
    rc = 0
    for rel in ('viewer/thumb.html', 'viewer/ly.html', 'viewer/k.html', 'viewer/track.html', 'viewer/conv.html'):
        try:
            html = open(os.path.join(ROOT, rel), encoding='utf-8').read()
        except Exception:
            continue
...
def check_soremeori():
    ...
    for rel in ('viewer/thumb.html', 'viewer/k.html', 'viewer/ly.html', 'viewer/track.html', 'viewer/conv.html'):
        try:
