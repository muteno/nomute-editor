...
def check_viewer_js():
    ...
    rc = 0
    for rel in VIEWERS_ALL:   # Q165 nb·sb 편입 → Q167 뷰어 목록 SSOT 상수화
        try:
...
def check_autocomplete():
    ...
    rc = 0
    for rel in VIEWERS_ALL:   # Q167 뷰어 목록 SSOT 상수화 — nb·sb 동반 편입
        try:
            s = open(os.path.join(ROOT, rel), encoding='utf-8').read()
        except Exception:
            continue
...
def check_tokens_link():
    ...
    rc = 0
    for rel in VIEWERS_TOOLS:   # Q167 뷰어 목록 SSOT 상수화 — nb·sb 동반 편입(둘 다 tokens.css 링크 실측)
        try:
            html = open(os.path.join(ROOT, rel), encoding='utf-8').read()
        except Exception:
            continue
...
def check_dangling_var():
    """댕글링 var() 하드게이트(평의회 Q165 게이트 갭 ① → Q167 신설).
    ...
    try:
        ...
        for rel in VIEWERS_ALL:
            ...
            for name in sorted(set(_VAR_USE_NOFB.findall(s)) - defs):
                print('❌ 댕글링 var(): %s 가 %s 를 폴백 없이 참조하는데 정의가 없음(자기 파일·tokens.css) → 토큰 추가(정본값 계승) 또는 폴백 인자(Q167)' % (rel, name))
                rc = 1
        if rc == 0:
...
def check_soremeori():
    ...
    for rel in VIEWERS_TOOLS:   # Q167 뷰어 목록 SSOT 상수화(구 순서 thumb·k·ly… = 동일 집합) — nb·sb 동반 편입
        try:
...
def main():
    ...
    try:
        if check_dangling_var() != 0:   # 댕글링 var() = 미정의 토큰 무폴백 참조(하드 — IACVT 무효 렌더 차단·평의회 갭① Q167)
            rc = 1
    except Exception as e:
