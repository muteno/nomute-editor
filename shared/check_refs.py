...
_DESIGN_BASELINE = {
    ...
    'viewer/comp.html': {'accent_raw': 0, 'blur': 2, 'hex': 1, 'accent_hex': 0, 'green_wash': 0, 'legacy_green': 0},   # 감사 배치3(260704): err빨강→var(--danger)[신설]·뜬회색#cfd2d7→--mut(hex1=textarea bg #0e0f11만). nm-svg.js 로드·인라인아이콘 5종 삭제·상태줄 이모지→SVG = icon_ssot·viewer_js 게이트 편입(반쪽편입 해소·분신술10 260704). # STAGE1: --accent-rgb 추가·raw 7곳 토큰화 → accent_raw 7→0(픽셀0·k/ly 패턴·260628).
    'viewer/track.html': {'accent_raw': 0, 'blur': 3, 'hex': 17, 'accent_hex': 1, 'green_wash': 0, 'legacy_green': 1},   # 트래킹 실연결 편입(평의회6 상 — 신규 뷰어 게이트 사각 봉합·260708). accent_hex1·legacy_green1 = JS PALETTE 인물색 배열(track_render.py PALETTE와 1:1 짝 = 산출물 색 상수·§핵심명령 3-b-1 정당 raw — 카드색≠영상색 드리프트 방지가 목적이라 var() 불가). blur3 = .urlclip 글래스 backdrop+webkit(ly 계승) + .ftype 1. hex17 = PALETTE 12 + 입력 bg #0e0f11×2 + filmstage #0a1a0d + vstage #000×2(ly 팔레트 계승 관행 내).
}
...
def check_viewer_js():
    ...
    rc = 0
    for rel in ('viewer/index.html', 'viewer/thumb.html', 'viewer/ly.html', 'viewer/k.html', 'viewer/comp.html', 'viewer/track.html'):
        try:
...
def check_tokens_link():
    ...
    rc = 0
    for rel in ('viewer/thumb.html', 'viewer/ly.html', 'viewer/k.html', 'viewer/comp.html', 'viewer/track.html'):
        try:
            html = open(os.path.join(ROOT, rel), encoding='utf-8').read()
        except Exception:
            continue
...
def check_soremeori():
    ...
    for rel in ('viewer/thumb.html', 'viewer/k.html', 'viewer/ly.html', 'viewer/comp.html', 'viewer/track.html'):
        try:
