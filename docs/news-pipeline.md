...
> 🕒 **대기열(관측·상태판) — 제출 기사 처리 추적 (260619·260621 제스처 갱신):** 뷰어 **뉴스요약 버튼 = 롱프레스(600ms)·연달아 두번 탭·PC 우클릭·발견성 배지(처리중 건수) → 열기 / 떠있을 때 한번 더 탭 → 닫기**로 대기열 팝업. `functions/api/pending.js`(GET·`GH_TOKEN`)가 GitHub를 라이브 조회해 **세 상태**를 종합 반환(읽기 전용·파이프라인 0 변경):
