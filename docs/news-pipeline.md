...
- **✅ 폰 경로 = 해결 (§기틀 보호 승인 + §🧪 5인 검증 · 260618)**. 폰 공유 = 2가지 입력 모두 처리:
  ...
  - **pending 포맷**: line1=URL 또는 `paste:<해시>`(불변·dedup·`head -n1`), 선택 `# title:`, 선택 `# body:` *이후 전체*=본문(가산·하위호환·`awk '/^# body:/{f=1;next}f'`+`iconv -c`로 소비, 20KB 캡). 정본 = 플레이북 §5-2 + `docs/termux-share.sh` + `analyze.sh` + `prompts/news-analysis.md`. ⚠️ URL경로 전제 = `python libiconv`(§3); 전문경로는 fetch 안 해 의존성 무관(iconv만 정리용).
