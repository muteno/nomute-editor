...
- **모델**: `claude-opus-5` 고정(생성/하드작업 유지). **effort = 검색경로(analyze·ask)만 medium**(env `PIPE_SEARCH_EFFORT` · opus5 medium ≈ 구세대 high급 품질에 지연·토큰 절감 = 요약 단축 축 · 260727 일괄 하향[구 high=260704] · max 헛사고 회피는 동일), 나머지 생성경로는 max. 분석 도구는 `WebFetch,WebSearch`만 허용.
