...
| 모달 헤더 띠 | `.modal-head`(정본 · 별칭 `.tool-h`/`.dlg-h`/`.ed-h`/`.msgpop .mh`/`.qpop .qh`/`.pmenu-h`/`.pin-head` · `.askhead`=border만) | — | 배경 `--modal-head-bg`(.5)+blur `--blur-l`+border-bottom `--line2` · **불투명 컨테이너(ask-family·editdlg)는 blur 생략**(동색이라 안 보이고 260701 번쩍 회피) · 앵커 팝업은 `margin:-컨테이너padding`으로 전폭 | — | — | 제목 좌 + 버튼묶음 우(`margin-left:auto` 래퍼) | `11px 16px`(앵커는 padding 상쇄) | `aria-labelledby` | index `.modal-head`(값=:root 셸토큰 4종·260704) |
