...
|---|---|---|---|---|---|---|---|---|---|
| 닫기 X | `.tool-x`(별칭 `.dlg-x/.ed-x/.askx/.mx/.qh-x/.fb-x/.sc-rsn-x/.askimg-x/.pin-x`) | `--btn-sm`(30) | glass `.06`+`--line` · **모양 2종(의도·단일화 X · 운영자 260704)**: 원형(`.dlg-x`·`.sc-rsn-x`·`.askimg-x`) / 둥근네모 9px(`.tool-x`·`.ed-x`·`.askx`·`.pin-x`·`.mx` — 다수) | **회전180 .92 + 강조색(`--accent` 레몬라임) 전환** = 전역 X 통일(색 transition .18s · 노란색 `--arm` 아님 · index 1407~1408 공용 `:active` · 운영자 260704) | path `M6 6 18 18M18 6 6 18` sw1.8 | 헤더 우측묶음 / abs 우상단 | — | `aria-label="닫기"` | index `.tool-x` |
...
| SNS 소스칩 | `.soc-chip`(base 중립)·`.lean-ll/l/mid/r/rr`(성향 5단)·`.src-naver`+`.ico`(픽토칩) | `--fs`10.5·pill · 픽토 svg 11px(`.ico svg`) | base 중립 `rgba(255,255,255,.06)` / 성향 5단=편향토큰(강진보 `--bias-l2` 파랑칩·**흰 글자**=소형 대비AA / 약진보 `--accent-5` 시안 / 중립 흰 / 약보수 `--bias-r1` / 강보수 `--accent-3` bg) / 네이버 = 약진보 시안(`--naver`=accent-5)+픽토 | — | — | `.soc-srcs` 카드 하단 | 텍스트 `2×8` / 픽토칩 `.ico` flex·gap3·padL7 | **성향 5단 색=편향토큰 재사용**(뉴스 편향바 정합·운영자 260704·웹검색 분류). **네이버=픽토+짧은 라벨**: 카페=`CAFE_SVG`(☕컵)+'카페'·블로그=`BLOG_SVG`(✒️펜)+'블로그'·지식iN=이름만·커뮤=텍스트. 픽토(innerHTML 신뢰상수)+라벨(textNode XSS안전). 강진보만 흰글자(파랑 3.38:1 실패 회피) | index `renderSns`·`SOC_LEAN_MAP`(ll/l/mid/r/rr)·`NAVER_CHIP`·`socLean`/`socSrc` (260704) |
| 모달 헤더 띠 | `.modal-head`(정본 · 별칭 `.tool-h`/`.dlg-h`/`.ed-h`/`.msgpop .mh`/`.qpop .qh`/`.pmenu-h`/`.pin-head` · `.askhead`=border만) | — | 배경 `--modal-head-bg`(.5)+blur `--blur-l`+border-bottom `--line2` · **`<dialog>` 컨테이너 backdrop-filter 전면 0(260704 규칙① · 프로스트=`::backdrop` blur7 + 이 헤더 띠 `--blur-l`만 · textarea 타이핑 재샘플 번쩍 원천차단)** · DIV 앵커 팝업(`::backdrop` 없음)은 컨테이너 blur 유지(유일 프로스트원) · 앵커 팝업은 `margin:-컨테이너padding`으로 전폭 | — | — | 제목 좌 + 버튼묶음 우(`margin-left:auto` 래퍼) | `11px 16px`(앵커는 padding 상쇄) | `aria-labelledby` | index `.modal-head`(값=:root 셸토큰 4종·260704) |
| 모달 탭 띠 | `.modal-tabs`(정본 · 별칭 `.tool-tabs`) · 탭=`.tooltab`(thumb `.tab` 계승) | — | 배경 `--modal-tabs-bg`(.4)+border-bottom `--line2` · `has-tabs`일 때만 | on=accent | 글자탭(픽토+라벨) | 헤더 아래·내용 위 | `9px 13px` | — | index `.modal-tabs`(영상 자막생성/영상프롬프트 · 이미지 썸네일/저작권/카드뉴스) |
...
| 픽토그램-온리 액션(발행본) | `.pubpop .pub-ico`(`.pub-share`/`.pub-rm`)·잠금 `.pinmark` | `--btn-sm`(30·히트 `::before -6px`=42) | **무도형**(배경·테두리 0) · **상시 의미색**(공유 `--accent`/삭제 `--danger` — rest 무채는 운영자 260703 폐지) | 공유=`data-motion="share"` 별자리 훑기 · 삭제=`:active` scale `--press-xs` | SHARE_SVG·`_TRASHSVG` | 행 우측 클러스터(각 `--btn-sm` 등폭·gap 0 균등) | svg 18px | disabled=`--mut` 40%·행높이 51px(`.pubpop .qlist` basis 정합) · 별자리 훑기 도입처 = 발행본 `.pub-share` + 요약 헤더 `.dlg-pdf`(okflash 초록 플래시와 병행·260703) · **요약 헤더 통일(260704)**: 공유 `.dlg-pdf`=텍스트('공유') 제거·픽토만 16px, 원문 `.dlg-h a`(`#src`)와 **34px 투명 아이콘버튼**으로 통일(둘 다 rest=`--mut`·hover=`--fg`) / 공유 클릭=발행창 열기를 **~0.28s 지연**해 별자리 훑기 가시화(즉시 열면 발행창이 헤더 덮어 모션 안 보임 · reduced-motion=즉시) / 원문 `#src`=`data-motion="rise"` **↗튐(ic-rise)** 위임 편입(옛 `.arr` arrnudge=요소 부재 dead 대체) · 발행 모달 `#pubdlg` = 정적 무채색(`.pub-go` 발행버튼 글자·`.pub-vis` 비공개 토글 = `--fg` 흰) · **게이지 채움·완료·테두리는 기존 accent 유지**(운영자 260704 '게이지는 강조색이 맞음' · 새 색 창작·기존 색 임의 제거 안 함 = §🎨 색 원칙) · **3아이콘 균등+X정렬 통일(260704)**: 잠금 `.pinmark`·공유·삭제 각 `--btn-sm`(30) 등폭·`pub-acts gap 0` → 글자중심 30px 균등 + 마지막 삭제가 헤더 X와 세로 일직선(= 아래 「팝업 행-액션 정렬」 표준) | index pubRowHtml(#1377 복원·260703) |
...
| 팝업 행-액션 정렬(끝버튼↔헤더 X) | 헤더 `.qpop .qh-xcell` ↔ 행 최우측 `.qrow .qact`(또는 `.pub-acts` 마지막 아이콘) | 셀·버튼 = `--btn-sm`(30·슬랙0) | — | — | — | 헤더 X셀·행 끝 액션셀 = **동일 폭(`--btn-sm`)·center·우변 3px 인셋** → 끝버튼(이동↗/삭제) 중심이 X 아래 **세로 일직선** | qrow gap 9 | — | 대기열 이동↗·발행본 삭제·메시지함 삭제 **3팝업 통일(운영자 260704)** · 옛 3분기 폐지(대기열 flex-start 44셀 / 발행본 flex-end / 메시지함 center = 팝업 간 7px 델타 근원 = 44셀−30버튼 슬랙 절반) · 행 액션 여럿이면 각 `--btn-sm` 등폭+gap 0 등간격 · **헤더 X 위치 = 팝업 우변 3px 고정(전 팝업 동일)** | index `.qpop .qh-xcell`/`.qrow .qact`(§🎨) |
