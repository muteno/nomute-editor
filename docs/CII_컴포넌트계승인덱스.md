...
| SNS 소스칩 | `.soc-chip`(base 시안)·`.lean-l/-r`(정치성향)·`.src-naver`(네이버·색통일)+`.ico`(픽토칩) | `--fs`10.5·pill · 픽토 svg 11px(`.ico svg`) | base `rgba(--info-rgb,.1)` / 네이버 = **`--naver`(#03C75A) 단일틴트 통일**(무게변주·점선 폐지·운영자 260704) · lean 파/빨 | — | — | `.soc-srcs` 카드 하단 | 텍스트 `2×8` / 픽토칩 `.ico` flex·gap3·padL7 | **네이버 = 픽토 + 짧은 라벨**: 카페=`CAFE_SVG`(☕컵)+'카페' · 블로그=`BLOG_SVG`(✒️펜)+'블로그' · 지식iN=픽토없이 이름만 · 커뮤=텍스트=소스명. 픽토(신뢰상수 innerHTML)+라벨(textNode=XSS안전) | index `renderSns`·`NAVER_CHIP`(cls/icon/label)·`socLean`/`socSrc` (260704) |
