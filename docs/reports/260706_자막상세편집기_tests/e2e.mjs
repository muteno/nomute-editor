...
let mdCalls = 0;   // 첫 요청만 404 = 대기(gdots) 상태 검증 창 확보(poll 5s 재시도 1회)
await page.route('**/ly_out/e2e1/subs.md*', r => { if (mdCalls++ < 1) return r.fulfill({ status: 404, body: '' }); r.fulfill({ contentType: 'text/markdown; charset=utf-8', body: MD }); });
...
// 2) 생성 → 편집기 렌더(기본 = 상세)
await page.click('#go');
await page.waitForFunction(() => document.querySelector('#go').classList.contains('wait') && !!document.querySelector('#go .gdots'), null, { timeout: 3000 });
ok(true, '생성 중 = 버튼 대기 상태(gdots · 평의회R2 C2)');
await page.waitForSelector('.lytabs.show', { timeout: 20000 });
ok(!(await page.locator('#go').evaluate(el => el.classList.contains('wait'))) && (await page.locator('#go').textContent()) === '자막 생성', '완료 = 버튼 원복');
await page.waitForFunction(() => window.scrollY > 40, null, { timeout: 3000 }).catch(() => {});
ok(await page.evaluate(() => window.scrollY > 40), '완료 랜딩 자동 스크롤(평의회R2 B2)');
ok(await page.locator('#lyTabN').isVisible() && (await page.locator('#lyTabN').textContent()) === String(SEGJ.segs.length), '상세 탭 조각수 뱃지(평의회R2 B1)');
...
ok(await page.locator('.lyseg[data-i="0"]').evaluate(el => el.classList.contains('sel')), '선택 카드 좌측 바(.sel · 평의회R2)');
...
ok(await page.locator('.lyseg[data-i="0"]').evaluate(el => el.classList.contains('land')), '병합 착지 펄스(평의회R2 C3)');
ok(await page.locator('#lyUndoBtn').isVisible(), '병합 되돌리기 칩(평의회R2 A2)');
await page.click('#lyUndoBtn'); await page.waitForTimeout(150);
ok((await page.locator('.lyseg').count()) === n1, '되돌리기 = 병합 취소(카드 복원)');
await page.locator('.lyseg[data-i="1"] .lyseg-a[aria-label="위 조각과 합치기"]').click();
await page.waitForTimeout(150);
...
ok(await page.locator('.lyseg[data-i="0"] .lyseg-t').evaluate(el => el.disabled && el.classList.contains('plain')), '미리보기 없음 = 시크 비활성·크롬 제거(평의회R2 B3)');
ok(await page.locator('#out').isVisible(), '복원도 개요 기본(일관)');
await page.click('#lyTabDet');
await page.screenshot({ path: OUTDIR + '3_복원.png', fullPage: true });

// 14) 편집 초기화 = arm 2탭 + 실행 후 되돌리기(평의회R2 A1)
await page.click('#lyReset');
ok((await page.locator('#lyReset').textContent()).includes('한 번 더'), '초기화 1탭 = 재확인 arm');
await page.click('#lyReset');
await page.waitForTimeout(200);
ok(!(await page.locator('.lyseg[data-i="0"] .ed-chip').first().textContent()).includes('체크'), '초기화 2탭 = 원본 전사 복귀');
ok(await page.locator('#lyUndoBtn').isVisible(), '초기화 되돌리기 칩');
...
// 14d) 군더더기 원클릭(평의회R2 E1) — 칩 하나를 '음'으로 만들어 결정론 보장 → 빼기 → 되돌리기
const fchip = page.locator('.lyseg[data-i="1"] .ed-chip').first();
await fchip.dblclick();
await fchip.evaluate(el => { el.textContent = '음'; });
await fchip.press('Enter'); await page.waitForTimeout(150);
await page.click('#lyFiller'); await page.waitForTimeout(200);
ok(await page.locator('.lyseg[data-i="1"] .ed-chip').first().evaluate(el => el.classList.contains('off')), '군더더기 = 필러 칩 제외');
ok((await page.locator('#status').textContent()).includes('군더더기'), '군더더기 status 고지');
ok((await page.locator('#lyFiller').getAttribute('aria-pressed')) === 'true', '군더더기 버튼 on 상태(모델 도출)');
await page.click('#lyUndoBtn'); await page.waitForTimeout(200);
ok(!(await page.locator('.lyseg[data-i="1"] .ed-chip').first().evaluate(el => el.classList.contains('off'))), '군더더기 되돌리기 = 복원');
...
// 16) 진행 이어받기(평의회R2 C1) — a) 창 안 pend = 리로드 후 poll 자동 재개 → 완료 + pend 해제
await page.evaluate(() => { localStorage.clear(); localStorage.setItem('nm_ly_pend', JSON.stringify({ out: 'ly_out/e2e1/subs.md', b: true, t0: Date.now() - 10000 })); });
await page.reload();
await page.waitForSelector('.lytabs.show', { timeout: 20000 });
ok(true, '이어받기(창 안 pend) = 리로드 후 폴 재개→완료');
ok(await page.evaluate(() => !localStorage.getItem('nm_ly_pend')), '완료 후 pend 해제');
// b) 오래된 pend(창 밖) + 결과 존재 = 1회 확인으로 이어받기(타임아웃 0 유실 · 검토4)
await page.evaluate(() => { localStorage.clear(); localStorage.setItem('nm_ly_pend', JSON.stringify({ out: 'ly_out/e2e1/subs.md', b: true, t0: Date.now() - 5 * 3600e3 })); });
await page.reload();
await page.waitForSelector('.lytabs.show', { timeout: 15000 });
ok((await page.locator('#status').textContent()).includes('이어받'), '이어받기(오래된 pend) 배너');
