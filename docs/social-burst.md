...
| `SOCIAL_TOPIC_GATE` | `1`(ON) | **포지티브 공론화 게이트**(검토6인 260704) — `CONTROVERSY` 신호어(갑질·학폭·논란·폭로·황당·위협·사망…)가 제목/네이버스니펫에 ≥1 있어야 통과. 옛 필터는 네거티브 컷(정치·노이즈)만이라 웹툰·아이폰·줄거리 등 순수 정보/밈이 통과(라이브 6/6 떡밥)했던 것 차단. `0`=옛 동작. ⚠️ 신호어 없는 순수 정보는 컷되므로 통과량 감소(레인 성격상 의도) |
