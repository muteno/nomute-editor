...
export async function onRequestPost({ request, env }) {
  ...
  if (body.opts && typeof body.opts === 'object') {
    ...
    for (const k of ['size', 'outline', 'pad']) { const v = body.opts[k]; if (typeof v === 'number' && Number.isFinite(v) && v > 0 && v <= 3) o[k] = Math.round(v * 1000) / 1000; }
    { const v = body.opts.dual_small; if (typeof v === 'number' && Number.isFinite(v) && v >= 0.3 && v <= 0.62 && o.lang === 'dual') o.dual_small = Math.round(v * 100) / 100; }   // 번역 줄 크기 계수(운영자 260728 편집기 번역 토글 재입히기 짝 — dual 한정 · 결측 = ly_burn 0.62 종전 바이트)   // 연속 축(운영자 260707 선택값): size=높이비 소수(0.035) · outline·pad=계수 배율 — size 문자열(s/m/l)은 위 루프와 타입 상호배타 · 의미 범위 재클램프는 ly_burn(size_frac/coef)
