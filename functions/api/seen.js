...
export async function onRequestPost({ request, env }) {
  ...
  const AXES = ['t', 'f', 's', 'mr', 'md', 'pr', 'kw'];   // t=긴급 seen · f=실패 seen · s=시스템 경보 이벤트 · mr=메시지 읽음 · md=메시지 치움 · pr=픽실패 해소(계정 종속 소급 감사 260717 — CLAUDE.md [4] 룰) · kw=키워드 알림 발송 dedup(<slug>#<3h버킷> = 첫1회+24h/3h 각 발송 크로스기기 정확히 1회) — 축 추가 = 이 배열에만
