...
//   입력: GET ?u=<링크> · 출력: { kind:'media'|'article', subs:true|false|null }
...
export async function onRequestGet({ request }) {
  ...
  if (kind !== 'media') return json({ kind, subs: null });

  const id = ytId(u);
  if (!/^[A-Za-z0-9_-]{5,20}$/.test(id)) return json({ kind, subs: null });   // 유튜브 외 미디어 = 자막 유무 미상(파이프가 실측)
