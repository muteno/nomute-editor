...
CROSS_RE = re.compile(r'논란|공분|역풍|고소|고발|소송|법원|재판|구속|입건|유출|폭로|사과|퇴출|하차|파문|청원|신상|악플|혐의|갑질|미투|표절')
...
def score(title, d):
    s = sum(d['dict'].get(t, 0) for t in set(tokens(title)))
    if CROSS_RE.search(title or ''): s += d['cross_bonus']
    return round(s, 2)
