...
4. **거울·게이트** = `구성도/base.css`(viewer :root 자동거울) + `shared/check_refs.py`(raw 값 baseline 초과 시 경고). raw 줄이면 baseline도 낮춰라.
...
### 12. 누름 scale — ⚠️ 고정값 아님, 맥락별 계승
누름 피드백 `transform: scale(...)`은 **요소마다 다르다**(범용 지침의 "항상 0.96"은 우리에 틀림). 우리 현실:
- 작은 아이콘 버튼/별: `.82` (`.sc-star`)
- 토글: `.92` (`.ts-toggle`)
- 일반 글래스 버튼: `.85` (`.sbtn`) · 푸시 `.95`
- 프로필: `.97` · 카드: `.99`(큰 면일수록 1에 가깝게)

**규칙 = 새 요소는 가장 가까운 기존 형제 값을 계승**(크기 클수록 1에 근접). 임의 0.96 박지 마라.
