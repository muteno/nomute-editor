...
2. **규칙·컴포넌트 정본 = `docs/디자인기틀_SSOT.md §0 규칙 요약`**(구 `CLAUDE.md §디자인 디자인시스템`은 260701 해체 → §0로 집약) — 계승=디폴트, 재설계 금지. 컴포넌트 인벤토리(`.sbtn`·`.tool-x`/`.dlg-x`·`<dialog>`+`history.pushState` 모달·scroll-snap 캐러셀·`.askclip` 복사/붙여넣기·SVG 아이콘 상수 `COPY/PASTE/CHECK/...`)를 **그대로 이식**한다.
...
5. **컴포넌트 인덱스(CII) = `docs/CII_컴포넌트계승인덱스.md`** — 정본 셀렉터·크기·:active·아이콘 표. 새 컴포넌트는 표 계승. **표·토큰에 없으면 임의로 만들지 말고 운영자에게 먼저 질문**(필요 이유 + 가장 가까운 기존 후보 제시) — 승인분만 제작하고 즉시 기틀 편입: :root 토큰→거울 재생성→CII 행 등재→baseline 사유(운영자 지시 260702). 시맨틱 아이콘 모션은 **위임 1핸들러** 자동 상속 — 개별 모션 코드 창작 금지(260702 규칙 · `docs/디자인기틀_SSOT.md §0` 참조).
...
**규칙 = 새 요소는 가장 가까운 형제 컴포넌트의 press *토큰 티어*를 계승, `transform:scale(var(--press-*,폴백))` 참조로 쓴다**(하나 바꾸면 그 tier 전부) — **raw scale 창작 금지**·임의 0.96 박지 마라. 예외(게이지·이력색·카드)는 셀렉터 화이트리스트 = `docs/CII_컴포넌트계승인덱스.md`. reduced-motion 무효화는 :root 전역 블록이 담당(개별 가드 신설 금지).
...
- **제안·시안·N안·튜닝 = §플레이그라운드 만지는 5요소**(실물 재현 미리보기·프리셋·슬라이더·선택값 복사·한계 각주 — 골격 = `docs/플레이그라운드_포터블.md` §3)로 제작. ⚠️ 위 §작업표준② 5요소(전후·장단점 = 변경 *결과 보고*용)와 동명이의·별개 축 — 제안/시안 = §플레이그라운드 5요소 · 결과 보고 = §작업표준② 5요소.
