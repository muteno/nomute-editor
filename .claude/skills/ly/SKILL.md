...
1. `apps/ly/00_지침_자막기_v2.8.md` — 입력 감지·STT 파이프라인(Whisper large-v3-turbo·로컬·키 불필요)·조각 단위 규칙(전 소스 짧은 조각·영한 1:1)·모드(분리 기본/통합/한국어 원본)·다화자 색점(🔴🔵·명확한 조각만·코드블록 밖)·SNS 감성 의역 2패스·표기·출력 골격(개요→표→복사 블록)·자가검증 전체 (정본)
