...
def detect(prompt):
    ...
    return multi, (cmd_lines if cmd_lines else lines[:8])
...
def append_ledger(root, sid, segs, stamp):
    ...
    if not os.path.exists(path):
        return False
    rows = '\n'.join('- ⬜ %s' % s[:300] for s in segs[:12])
    block = '\n### 🧵 [%s %s] 훅 기계 캡처 — 원문 손대지 말고 [해석]·[계획] 붙여라\n%s\n' % (
        sid, stamp, rows)
...
def main():
    ...
    try:
        if len(prompt) >= 40 and not prompt.rstrip().endswith('?'):
            ...
            if multi:
                captured = append_ledger(root, sid, segs, now.strftime('%H:%M'))
                where = 'docs/요구사항_큐.md 말미에 기계 등재됨(⬜)' if captured else '큐 파일 부재 — 직접 등재하라'
                out.append(
                    '[📒 지시 복창 — 다중 지시 %d건 감지 · 실행 계약 2] 원문은 %s. '
                    '착수 전 각 ⬜에 [해석](왜 시켰나·표면 아님 목적)·[계획]을 채우고, 종료 시 ⬜→✅ 1:1 매트릭스 '
                    '+ 개수 자가검증(원장 N = 보고서 N섹션). 하나도 흘리지 마라 — 못 끝낸 건 ⬜로 남긴다("다음에" 금지).'
                    % (len(segs), where))
    except Exception:
