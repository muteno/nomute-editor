...
3-4. **픽토그램·버튼 신설/이동/정렬 = 픽셀 실측 의무(운영자 260716 "항상 픽셀단위로 4분할 중앙선 같은지 확인")**: 눈대중·CSS 산식 추정으로 "정렬됨" 선언 금지 → Playwright(/opt/pw-browsers/chromium) 실렌더에서 getBoundingClientRect 중심 비교 = {픽토 중심 = 버튼 4분할 중심 · 정렬 축 형제와 Δ≤0.5px(목표 0.00 — 홀짝 패리티로 정수 스냅)} + 중심 십자선 오버레이 캡처를 완료 보고에 증빙 첨부. ⚠ 미디어 티어 함정: CSS 정본 수치만 믿으면 틀린다(실측 260716 — .feednav 폰 25px vs hover PC 42px 티어, 산식이 25 기준이라 4px 편심났던 것을 실측이 잡음).
