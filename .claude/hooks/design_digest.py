...
if '--if-ui-prompt' in sys.argv:
    ...
    print('[🎨 UI 작업 감지 — 노뮤트 디자인 계약 리마인더] ⭐생성 1순위 = 기존 레퍼런스 100% 모방(부품·값+구성·배치·문법 "그대로" · 착수 1줄에 「레퍼런스 = X 그대로」 지목 · 정 없으면 만들지 말고 시안 선행 · 판정 예스/노: 99%=0%·새 모양=fail · 운영자 260720 — 정본 = CLAUDE.md 최우선 4). '
          ...
          '📸 완료 보고 = **전/후 캡처 첨부 의무**(운영자 260731 "항상 이미지로 전후를 띄워줘야하는데, 링크말고") — '
          '`node shared/shot_ba.js --before <베이스ref> --route <시나리오>` 로 같은 조건 두 판을 찍어 SendUserFile로 **이미지 자체**를 올린다. '
          '배포 URL·PR 링크로 갈음 금지(운영자가 눌러서 확인하게 만들지 마라) · 신규 화면은 shot_ba.js ROUTES에 한 줄 편입.')
    sys.exit(0)
