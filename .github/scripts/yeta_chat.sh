...
export CLAUDE_BARE=0   # 방어 명시 — claude_meter 공유 기본값이 미래에 ON 회귀해도 챗은 --bare 불가(평의회① · OAuth 즉사 차단)
...
# ⚠️ 404(키 부재)만 정상 종료 — 네트워크 블립까지 exit 0 하면 그 턴이 그린으로 은닉 유실(평의회① B).
if ! _gerr="$(r2get 2>&1)"; then
  if printf '%s' "$_gerr" | grep -qiE 'Not Found|NoSuchKey|404'; then
    echo "세션 없음(${KEY}) — 처리할 메시지 없음"; exit 0
  fi
  echo "::error::R2 세션 읽기 실패(일시 오류 추정 — 재전송으로 재시도): ${_gerr}"; exit 1
fi
...
print(json.dumps({"note": s.get("note") or "", "hist": hist, "pending": "\n".join(pending),
                  "last_a": last_a, "pend_n": len(pending)}, ensure_ascii=False))   # 답장 insert 위치용(평의회⑦ 매몰 방지)
PY
)"
[ "$mat" = "NOPENDING" ] && { echo "미응답 메시지 없음 — 종료(연속 dispatch 큐 정상)"; exit 0; }
[ -n "$mat" ] || { echo "::error::세션 파싱 실패(malformed JSON) — state 미변경 종료(평의회① C)"; exit 1; }
...
LAST_A="$(python3 -c 'import json,sys; print(json.loads(sys.argv[1])["last_a"])' "$mat")"
PEND_N="$(python3 -c 'import json,sys; print(json.loads(sys.argv[1])["pend_n"])' "$mat")"
...
for attempt in $(seq 1 "$INLINE_TRIES"); do
  out="$(printf '%s' "$prompt" | METER_SRC=yeta METER_REF="$CHAR" METER_MODEL="$MODEL" METER_EFFORT= claude_meter 240 \
        --model "$MODEL" $SAFE \
...
# ── 결과를 세션에 반영 — 커밋 직전 재-read(fresh) 후 반영(경합 창 최소화 · 계획안 §7) ──
# ⚠️ 답장은 끝-append 가 아니라 "답한 유저 턴 바로 뒤"(LAST_A+1+PEND_N)에 insert — 생성 중 도착한 후속 메시지가
#    assistant 뒤 인덱스에 남아 다음 런의 pending 으로 잡힘(끝-append 는 후속 메시지를 영구 무응답으로 매몰 · 평의회②⑦).
#    잔여 pending 있으면 state=awaiting 유지(뷰어 폴 지속 · 큐잉된 생존 런이 드레인). rc=2 = 세션 교체 감지(reset 레이스) → 폐기.
finish() {  # $1 = ok|error · $2 = 답장 텍스트(ok) / 에러 요지(error)
  r2get || : > /dev/null   # fresh 재-read(그 사이 유저 메시지 append 보존). 실패 시 기존 $SESS 로 진행(비치명)
  REPLY_TEXT="$2" python3 - "$SESS" "$1" "$CVER" "$LAST_A" "$PEND_N" <<'PY'
import json, os, re, sys, time
p, kind, cver, last_a, pend_n = sys.argv[1], sys.argv[2], sys.argv[3], int(sys.argv[4]), int(sys.argv[5])
s = json.load(open(p, encoding="utf-8"))
turns = s.setdefault("turns", [])
now = int(time.time() * 1000)                     # 저장 = epoch ms(무모호 · KST 는 표시 계층 · 계획안 §8)
ins = last_a + 1 + pend_n                         # 답장 자리 = 이번에 답한 유저 턴들 바로 뒤
if len(turns) < ins:                              # fresh 가 더 짧다 = 그 사이 reset(세션 교체) → 이 답장은 옛 대화 것 = 폐기(평의회② 3-b)
    print("세션 교체 감지(reset 추정) — 이 답장 폐기", file=sys.stderr); sys.exit(2)
text = os.environ.get("REPLY_TEXT", "")
note, empty = None, False
if kind == "ok":
    m = re.split(r'<<\s*NOTE\s*>>', text, maxsplit=1, flags=re.I)   # 공백·대소문자 변형 관대(마커 변형 시 노트가 대사로 유출되던 갭 · 평의회⑤)
    if len(m) == 2:
        text = m[0]
        note = re.split(r'<<\s*/\s*NOTE\s*>>', m[1], maxsplit=1, flags=re.I)[0].strip()[:1200]
    text = text.strip()
    if not text:                                  # 노트만 있고 대사 0 = 빈 말풍선 방지 → error 폴백(재전송 유도 · 평의회⑤ 갭B)
        s["state"] = "error"; s["err"] = "빈 대사(모델이 대사 없이 응답) — 다시 보내면 재시도"; empty = True
    else:
        turns.insert(ins, {"role": "assistant", "text": text, "ts": now})
        if note:
            s["note"] = note
        # 잔여 pending(생성 중 도착한 후속 유저 턴) 있으면 awaiting 유지 — 큐잉된 생존 런이 이어서 답함
...
PY
  _frc=$?
  case "$_frc" in
    2) echo "세션 교체(reset) — 반영 생략"; return 0 ;;                                          # 옛 대화 답장 = 폐기
    3) r2put || return 1; echo "::warning::빈 대사 — error 상태 기록(푸시 생략)"; return 0 ;;      # 마커 안 만듦 = 헛푸시 방지
    0) r2put || return 1
       [ "$1" = "ok" ] && : > /tmp/yeta_replied                                                   # 실답장 마커 — 푸시 게이트(평의회⑦⑧)
       return 0 ;;
    *) echo "::error::세션 반영 파이썬 실패(rc=$_frc)"; return 1 ;;
  esac
}
...
finish ok "$out" || { echo "::error::세션 반영 실패(R2 put) — 답장 미저장"; exit 1; }
