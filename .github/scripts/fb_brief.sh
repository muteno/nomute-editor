...
- [총론] = 페이지 전체를 아우르는 비전·방향성·미션 — 개별 수치 나열 말고 정체성·나아갈 큰 방향(3~12개월)·핵심 미션을 산문으로. **[팔로워 인구통계]가 데이터에 있으면 '이 페이지의 관객이 누구인가'를 총론 방향에 반드시 녹여라**(운영자 260724): 연령대 폭·성별 구성을 읽어 소재·톤 방향의 근거로 삼되(예: 연령대가 넓거나 고연령이면 차분한 정리형 등 그 관객에 맞는 방향), 성별은 '눈대중 추정' 딱지대로 단정 말고 방향 참고까지만. **맨 마지막 줄은 '→ '로 여는 단 한 줄 결론**('→ 그래서 무엇을 — 한 문장 방향'). 6~9줄.
