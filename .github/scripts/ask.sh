...
' 2>/dev/null || true)"
    if [ -n "${_su// }" ]; then
      ...
      if [ -n "$srcimglist" ]; then
        # ── OCR 추출(운영자 260804 2차 "사진안에 문자열을 토대로 가져오게 배선 · 뉴스 요약에 연결") ──
        #   그림을 **첨부만** 하면 본선이 그걸 여는지가 프롬프트 준수 확률에 달리고, 열어도 '전문(원문)'의
        #   지위를 못 얻는다 — 프롬프트 1)의 보강 모드는 「요청문에 전문이 있으면」 발동하는 축이라
        #   그림만으론 그 레일에 안 물린다. → 문자열을 확정적으로 뽑아 **미디어 전사문과 같은 지위**로
        #   실으면(ask_link_stt.sh 「이 전사문이 곧 원문이다」 문법 계승) 기존 뉴스 요약 로직에 그대로 연결된다.
        #   fail-soft 2단 = OCR 실패해도 그림 첨부 블록(구 동작)으로 내려앉는다 = 악화 경로 0.
        _ocr=""
        if [ "${ASK_SRCOCR:-1}" != "0" ]; then
          _ocr="$(timeout "${ASK_SRCOCR_TIMEOUT:-360}" python3 .github/scripts/ask_srcocr.py "${srcimgs[@]}" 2>/dev/null || true)"
          echo "· 본문 이미지 OCR: $([ -n "${_ocr// }" ] && echo "${#_ocr}자 추출(${#srcimgs[@]}장)" || echo '추출 0 — 그림 첨부로 폴백(fail-soft)')"
        fi
        if [ -n "${_ocr// }" ]; then
          SRCIMG_BLOCK="[🖼 출처 글의 본문 — **그림뿐인 게시글이라 그림 안의 글자를 추출했다**(${_su}). ⭐ **이 추출문이 곧 원문이다** — 위 1)의 보강 모드에서 말하는 '전문'으로 취급하라(미디어 링크 전사문과 같은 지위).
 ...
 - 관련 보도가 없는 유머·일상·후기·화면캡처성 화제면 **기사를 억지로 찾지 마라** — 그 게시물 자체가 소재다. 추출문 범위 안에서 완성하라(frontmatter url = 이 글 주소).
 - ⚠️ 추출문에 없는 사실·수치·인용은 지어내지 마라. \`[N]\` 은 이미지 번호 표시일 뿐이니 본문에 옮기지 마라. \`(글자 없음)\`·\`(본문 아님 — 광고/배너)\` 항목은 무시하라.
 - 원본 그림도 아래에 붙여 뒀다 — 추출문이 애매하거나 표·그래프처럼 글자만으로 안 잡히는 게 있으면 Read 로 교차 확인하라.
