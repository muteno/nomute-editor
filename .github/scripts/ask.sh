...
[★ 요약 요청 모드 — 운영자가 자연어 + 캡처로 큐레이션을 직접 요청했다.
 ...
 1-2) ⭐ **요청문이 국내 커뮤니티 게시글이면(제목 + 커뮤니티 URL[fmkorea·theqoo·dcinside·mlbpark·clien·인스티즈 등], 본문 없음·짧은 스니펫만) = 커뮤니티 원문은 봇 차단으로 못 열릴 수 있으니 제목의 핵심 키워드로 WebSearch 해 같은 화제를 다룬 메이저 기사를 찾아 요약하라**(대상 = 게시글이 아니라 그 화제 · 커뮤니티 제목 특유 표현 .jpg·ㅋㅋ·밈·축약은 핵심 키워드로 정제해 검색). 이 폴백은 자동이다 — 원문 fetch 성공 여부와 무관하게 화제의 메이저 보도를 우선한다(운영자 260729). 검색 상한(2~3회)·최신 우선(18시간)은 위와 동일.
