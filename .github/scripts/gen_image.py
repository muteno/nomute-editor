...
def ask_opus(head, lead, insight, scene, o):
...
한줄 요약: {lead}
시사점: {insight}
장면 제안(분석 시점 초안 — 더 좋은 결정적 장면이 있으면 재구성 가능): {scene}
...
[작성 규칙]
- 이 사건을 즉시 알아보게 하는 결정적 순간 하나 — 막연한 무드샷·스톡사진 포즈·카메라 보고 웃는 인물 금지.
- {person}
...
def main():
    ...
    print("🎨 이미지 생성 — '{}' · 화풍={} 비율={} 해상도={} 장수={}{}{}".format(
        head[:40], STYLE_KO[o["style"]], o["aspect"], o["size"], o["count"],
    ...
    for i in range(o["count"]):
        ...
        url = None
        if tg.R2_ON:
            url = tg.r2_upload(png, "thumbs/{}/genimg-{}-{}.png".format(stem, h8, i + 1))
        if not url:   # R2 미설정/실패 = git 폴백(로컬 커밋 → 뷰어 상대경로 서빙·gen.json 폴백과 동일 방식)
            fname = "genimg-{}-{}.png".format(h8, i + 1)
            with open(os.path.join(tdir, fname), "wb") as f:
                f.write(png)
            url = "cards/{}/thumbs/{}".format(stem, fname)
            print("  ⚠️ R2 불가 — git 폴백 저장: " + url, flush=True)
        new_items.append({"url": url, "link": "", "label": "생성",
                          "style": o["style"], "prompt": prompt[:1500]})
        print("  ✅ {}/{} → {}".format(i + 1, o["count"], url), flush=True)

    if not new_items:
        die("렌더 전건 실패 — 생성 이미지 0")
    json.dump(new_items + existing, open(sjson, "w", encoding="utf-8"), ensure_ascii=False, indent=2)
