...
body = '\n'.join(L)
PVER = 'brief-v4-260712'   # 캐시 1회 무효화 = v4: 분석 프레임(크로스플랫폼·이상치·so-what)은 유지 + 트렌드 브리핑체(사장님·비서 어투 제거·표준편차 벗어나는 것만 강조 · 운영자 260712) · 다이제스트 fmt_view 만단위 사전변환
...
[말투]
담백한 트렌드 브리핑체 — '~가 지금 화제를 모으고 있다' '~로 관심이 쏠린다' 식으로 무슨 주제에 관심이 쏠리는지 중심으로 써라. 인사말·비서 어투·호칭('사장님' 등) 절대 금지. 딱딱한 보고서체도 금지.
...
[좋은 예 — 분석·담백(이렇게 하라)]
지금은 *축구가 세 플랫폼을 다 먹었다* — 틱톡·구글검색에 월드컵 관련이 동시에 떠서 화력 1순위다.
다만 *틱톡 1위는 조회 8,970만짜리 한 계정의 폭발*이라, 주제 유행이라기보단 단일 바이럴로 보인다.
유튜브는 결이 달라 *K-엔터·e스포츠로 쏠리는데*, 영상 오디언스가 검색·틱톡과 갈린다는 신호다.
