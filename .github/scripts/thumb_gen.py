...
STYLES = [
    ...
    ("photo_close", "포토 에디토리얼 — 클로즈업",
     "보도/르포르타주 사진 스타일. 자연광, 현장 다큐멘터리 사실감. "
     "스튜디오 인물사진·잡지 화보가 아니라 현장에서 포착된 보도사진의 클로즈업. "
     "핵심 인물·사물의 타이트한 클로즈업, 얕은 심도 보케로 배경 흐림, 표정·눈빛·디테일의 결정적 순간, 살짝 로우앵글."),
    ("cartoon", "시사만평",
...
# full-bleed 충전 + (구) '하단 자막 자리' 폐기(다운스트림에 썸네일 자막 미존재 = 무용·검정 띠만 유발 — 260621 분신술 실측) + 안전 최우선.
COMPOSITION = (
    ...
    "[안전 최우선 — 아래는 어떤 긴장·극적 연출보다 우선] 사고·재난·범죄·사망 사안이라도 시신·유혈·신체 훼손·피해자의 고통을 직접 묘사하지 말고 현장·맥락·상징으로 암시한다. "
...
def parse_md(path):
    ...
    if iq in ("삼성전자 반도체 평택공장",):   # 프롬프트 예시값을 모델이 그대로 베끼면 무시(→ head 폴백)
        iq = ""
    return head, lead, iq, fm.get("url", "").strip(), fm.get("alt_urls", "").split(), fm.get("image_sources", "").split(), fm.get("thumb_dispatch", "").strip()
...
def process_one(md, stem):
    ...
    head, lead, iq, art_url, alt_urls, image_sources, dispatch = parse_md(md)
    if not head:
    ...
    for sid, label, art_dir in STYLES:
        ...
        png = gemini_image(build_prompt(art_dir, iq or lead, dispatch), "2K")   # 장면 = entity(iq) 우선·없으면 한줄요약 + 라이브러리 앵글/조명/연출(dispatch) 삽입
        if not png:
