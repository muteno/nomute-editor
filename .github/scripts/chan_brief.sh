...
body = '\n'.join(L)
PVER = 'chanbrief-v4-260714-3d-no14d'   # 프롬프트 버전 — 바뀌면 해시 불일치 = 다음 run 강제 재생성 · v4 = 3일 신설·14일 제거(운영자 260714 2차) · v3 = 기간 5부 구성(운영자 260714 "구분해서 요약") · v2 = 서두 프리앰블 금지(카나리아 봉합)
...
[3개월]
[전체 총론]
- [3일] = 지난 사흘 속보. 여는 인사 한 줄로 시작 → 막 벌어진 급변(급증·급락)과 그걸 끌고 간 게시물을 콕 — 짧고 기민하게. 3~4줄.
