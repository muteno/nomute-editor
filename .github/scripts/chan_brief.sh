...
body = '\n'.join(L)
PVER = 'chanbrief-v6-260714-follower-hours'   # 프롬프트 버전 — 바뀌면 해시 불일치 = 다음 run 강제 재생성 · v6 = 팔로워 활동 시간대 총론 반영(API 공회신 → 수기 실측 폴백 + 시간 분포) + 게시 요일 교란 딱지(운영자 260714 4차 "시간대는 총론 반영 가능·요일은 게시 몰림 영향") · v5 = 존재 이유(추이 비교→전략·창별 시사점 줄) + 직전 브리핑 연재 동봉 + 아카이브 · v4 = 3일 신설·14일 제거 · v3 = 기간 5부 · v2 = 서두 프리앰블 금지
...
- [전체 총론] = 처음부터 지금까지 성장 스토리 총정리(처음 보는 사람·초등학생도 '아, 이 채널 이렇게 커왔구나'가 한 번에) + 관리 전략 '→ '로 시작하는 줄 3~4줄(각 줄 = '→ 무엇을 하자 — 근거(수치)' 꼴 · 예: '→ 릴스 비중을 더 올리자 — 릴스가 피드보다 1천뷰당 공유가 ×1.7 높다.' · 뻔한 일반론 금지 — 반드시 이 데이터에서만 나올 수 있는 말로) + 맺음 한 줄. 7~10줄. ⚠ '→ ' 줄들은 앱이 '클로신의 제안' 블록으로 자동 분리 표시한다(운영자 260714) — '전략은 이렇다:' 류 예고 문장 없이 줄만 쓰고, 맺음 한 줄은 '→ ' 없이 써라(맺음까지 제안 블록에 끌려가는 것 방지).
- 각 섹션 = 그 기간 창 데이터([기간 창별 실측]·일일 흐름·TOP·최근 게시물)가 근거. 섹션 간 같은 문장 복붙 금지 — 창이 넓어질수록 시야도 넓어지게, 앞 창과의 시야 차이(같은 데이터가 달리 읽히는 지점)를 살려라.
...
SECS = [('d3', '3일'), ('d7', '7일'), ('d28', '28일'), ('m3', '3개월'), ('all', '전체 총론')]   # 3일 신설·14일 제거(운영자 260714 2차)
