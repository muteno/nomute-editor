...
body = '\n'.join(L)
PVER = 'chanbrief-v9-260715-whip-sample'   # v9 = 회초리·표본(운영자 260715 Q02·Q03 — 인과 실측·팔로워 표본·반응 지문·확장문)   # 프롬프트 버전 — 바뀌면 해시 불일치 = 다음 run 강제 재생성 · v8 = 총론 분리(운영자 260714 "총론=비전·방향성·미션 큰 그림 3~12개월 / 전체=전체 기간 분석 디테일" — [전체 총론] 1부 → [전체]+[총론] 2부 = 6부) · v7 = 강조 2층 · v6 = 시간대·요일교란 · v5 = 존재이유+연재+아카이브 · v4 = 3일신설 · v3 = 5부 · v2 = 프리앰블금지
