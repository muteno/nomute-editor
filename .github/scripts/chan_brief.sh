...
# 페르소나 = "채널을 같이 키우는 친한 그로스 애널리스트 · 호칭·이름 없이 친근 인사(KST) · 성장 3기 서사 → 최근 급변 원인 콕 → 데이터 근거 실행 전략 · 쉬운 말 · 수치 신뢰선 사수"(sns_brief v8 톤 계승).
...
body = '\n'.join(L)
PVER = 'chanbrief-v2-260714-nopreamble'   # 프롬프트 버전 — 바뀌면 해시 불일치 = 다음 run 강제 재생성 · v2 = 서두 확인 멘트·'---' 구분선 금지(카나리아 1차 실측: 리서치 확인 프리앰블+수평선이 본문에 섞임 → 프롬프트 가드+후처리 봉합)
...
[핵심 임무 3가지 — 전부 · 이 순서로]
1. **성장 스토리**: 성장 3기·운영자 관측 변곡 이벤트·일일 흐름을 맞물려, 이 채널이 언제 잘나갔고 언제 왜 꺾였고 지금 어디쯤인지 한 편의 흐름으로 풀어라. 처음 보는 사람·초등학생도 '아, 이 채널 이렇게 커왔구나'가 한 번에 그려지게.
2. **지금 벌어지는 일**: 최근 7일이 전 기간 평균 대비 어떤지, 뭐가 튀었는지(급증·급락), 그걸 끌고 간 게시물이 뭔지 TOP·최근 게시물에서 콕 집어라. 게시물 소재(사건)가 원인 이해에 필요하면 WebSearch로 그 사건을 확인해 한 줄로 붙여라(확인된 것만).
3. **관리 전략**: 데이터 근거를 콕 집은 실행 전략을 줄 단위로 '→ '로 시작해 3~4줄. 각 줄 = '→ 무엇을 하자 — 근거(수치)' 꼴. 예: '→ 릴스 비중을 더 올리자 — 릴스가 피드보다 1천뷰당 공유가 ×1.7 높다.' 뻔한 일반론(꾸준히 올리자 류) 금지 — 반드시 이 데이터에서만 나올 수 있는 말로.
...
[형식]
- 응답은 여는 인사 문장으로 **즉시** 시작하라 — '확인됐어/찾아봤어/이야기 풀게' 류 준비·확인 멘트, '---' 같은 구분선, 서두 사족 전면 금지(그건 네 사고 과정이지 브리핑이 아니다).
- 자연스러운 문단 흐름: 인사 → 성장 스토리 → 지금 벌어지는 일 → 전략 줄들('→ ') → 맺음 한 줄. 10~18줄 안팎.
- 제일 크게 튄 수치·전환점은 *별표*로 강조(진짜 특별한 것만 · 별표 사이 줄바꿈 금지).
- 헤더·번호목록·마크다운 제목·이모지 금지(전략 줄의 '→ '만 예외).
...
# 줄바꿈 보존(요점별 개행 · sns_brief 계승) — 줄별 trim + 빈줄 3+ → 1 + 독해 상한(성장 서사+전략 = 길어짐 · 과출력 가드)
...
json.dump({'text': t, 'updated': datetime.datetime.now(KST).isoformat(timespec='seconds'),
          ...
          open('viewer/chan_brief.json', 'w', encoding='utf-8'), ensure_ascii=False)
print('chan-brief 저장:', len(t), '자', '·', t.count(chr(10)) + 1, '줄')
PY
