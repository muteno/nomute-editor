...
body = '\n'.join(L)
PVER = 'chanbrief-v5-260714-trend-serial'   # 프롬프트 버전 — 바뀌면 해시 불일치 = 다음 run 강제 재생성 · v5 = 존재 이유(추이 비교→전략·창별 시사점 줄) + 직전 브리핑 연재 동봉 + 아카이브(운영자 260714 3차 "이슈 감시 아니라 전략·모으면 뭔가 나올수도") · v4 = 3일 신설·14일 제거 · v3 = 기간 5부 · v2 = 서두 프리앰블 금지
