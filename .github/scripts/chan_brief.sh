...
body = '\n'.join(L)
PVER = 'chanbrief-v7-260714-2tier-hours'   # 프롬프트 버전 — 바뀌면 해시 불일치 = 다음 run 강제 재생성 · v7 = 강조 2층(*별표1*=강조색 / **별표2**=볼드만 · 운영자 260714 "어느정도 중요한 부분은 강조색 안쓰고 볼드로") + [v6 병합] 팔로워 활동 시간대 총론 반영·게시 요일 교란 딱지 · v5 = 존재 이유(추이 비교→전략)+연재+아카이브 · v4 = 3일 신설·14일 제거 · v3 = 기간 5부 · v2 = 서두 프리앰블 금지
