...
⚠️ 단 그 기상으로 인한 **실제 피해·긴급 대응이 제목에 명시**되면 이 게이트 미적용 — 위 일반 규칙으로 판정
(사망·다수 온열질환/한랭질환 이송·대규모 대피·단수·정전·산불·붕괴·고립·침수/범람·홍수·산사태 등 2차 사건).
⚠️ **재난 자체의 발생·임박·실황**도 이 게이트 비대상(관측 수치가 붙어도 위 일반 규칙으로 판정) —
태풍·호우의 상륙·직격 실황, 극한호우·침수 임박(«시간당 100㎜ 물폭탄»·«하천 범람 위기»),
지진·화산·지진해일(쓰나미) 등 지질 재난의 규모·특보·경보(«규모 4.7»·«지진해일주의보» = 기상 관측 보도가 아님).
단 태풍 발생·먼바다 북상·진로 전망(«한반도 영향은?»류)은 전망 보도 = X.
  ...
  예) "40도 폭염에 물도 바닥…청도군 일부 단수" → 단수 발생 = 판정 · "폭염에 온열질환자 30명 이송"(한 현장 동시다발) → 다수 피해 = 판정
  예) "[속보] 태풍 ○○ 통영 인근 상륙" → 재난 실황 = 판정 · "[속보] 서울 시간당 100㎜ 물폭탄…도로 곳곳 침수" → 판정 · "제12호 태풍 발생…한반도 영향은?" → 전망 = X
...
- ⚠️ 🌐(해외 군사)·🔪·📈·🔎·🌡 게이트는 **언어 무관 그대로 적용**(영문이라고 완화 아님 — «hottest day on record»류 영문 기상 신기록도 🌡로 X).
