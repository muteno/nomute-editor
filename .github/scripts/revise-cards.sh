...
MODEL="$PIPE_MODEL"
source "$ROOT/shared/claude_transient.sh"   # is_quota/claude_failover — 계정 한도 시 대체 계정 1단계씩 전환(서브1→서브2→서브3)
...
[★ 카드 수정 요청 모드 — 운영자가 이미 만들어진 '카드뉴스 프롬프트(cards.md)'의 진행 방식·맥락이 마음에 안 든다며 재기획을 요청했다.
 ...
 ⚠️ **한 줄 최대치(합성기 하드 상한 — 넘으면 그 카드는 렌더 자체가 중단된다)**: 카드 텍스트는 자동 줄바꿈이 없다. **한 줄 ≤ 환산 18.5자**(1자=50px · 한글·한자·\`%\`=1.0 / 숫자·영문·큰따옴표·가운뎃점·물음표=0.65 / 공백·마침표·쉼표·작은따옴표=0.3 · \`*강조*\` 별표 제외) = **한글만이면 ≤18자, 공백 포함 실전 상한 한글 17자·목표 15~16자**. 줄 ≤4. 원본에 이미 넘치는 줄이 있으면 이번에 줄여라(위 지침 [합성기 호환] 그대로).
...
# 규격 린트(비차단 · 260728) — 이 경로엔 게이트가 아예 없어서, 재작성이 한 줄 폭 상한을 넘겨도
#   조용히 저장됐다(cardmake 생성 경로만 lint가 있었다). 여기선 차단하지 않고 *가시화*만 한다
#   (운영자 지시가 원본이라 되돌릴 기준이 없음 — 슛 전에 '텍스트만 수정'으로 교정할 신호).
lint_out="$(python3 .github/scripts/card_gate.py lint "$CARDS" 2>&1)" || {
  echo "::warning::[$FILE] 재작성본 규격 위반 — 슛하면 합성이 중단될 수 있다. '텍스트만 수정'으로 교정 권장"
  printf '%s\n' "$lint_out" | grep '^LINT ✗' | sed 's/^/  /'
}
