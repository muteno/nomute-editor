...
# 카드 프롬프팅 추론 강도(운영자 260722 · C안 카나리아) — 실측: max의 출력 86~92%가 사고토큰(몇 문장 기사 → 실카드 ~1만tok인데
#   앞서 ~9만tok을 혼자 고민 = 23분). 카드 텍스트 = max-effort analyze가 이미 정제한 요약본을 '재작성'하는 변환 작업이라 high로 충분.
#   lint/coverage 게이트가 품질 방어 · 메트릭 dur_ms로 효과 관측 · 롤백 = CARD_EFFORT=max(무검증 저하 시 한 변수).
