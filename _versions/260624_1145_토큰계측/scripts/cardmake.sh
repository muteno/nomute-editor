...
$(cat "$q")"
    ...
    for attempt in $(seq 1 "$INLINE_TRIES"); do
      out="$(printf '%s' "$fp" | timeout 900 claude -p \
            --model "$MODEL" \
            --effort max \
            --allowedTools "WebFetch,WebSearch" \
            --disallowedTools "Write,Edit,MultiEdit,NotebookEdit,Bash,Task,Read,Glob,Grep" \
      ...
      if claude_failover "$out$(cat "/tmp/${stem}.err" 2>/dev/null)"; then continue; fi   # 쿼터 한도 → 대체 계정 1회 전환·재시도(account failover · SSOT)
    ...
    claude_health_update "$out" "/tmp/${stem}.err"   # 응답O=정상(경고해제) / 빈응답+인증·쿼터=경고(프로필 점등)
