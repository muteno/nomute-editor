...
# ── edit 모드: 단일 카드 재발사(텍스트·이미지 희망 → 그 카드만 재렌더·제자리 교체) ──
...
if [ "$MODE" = edit ]; then
  ...
  echo "::group::카드 변경 재발사: $stem 카드$CARD_N"
...
print(t or 'news')" 2>/dev/null)"
  ...
  # 3) 그 1장만 렌더(--out 임시) — drive_cards 그대로(Apps Script→Gemini→합성)
  python3 .github/scripts/drive_cards.py --md /tmp/onecard.md --topic "${topic}_c${CARD_N}" --out /tmp/reshoot; rc=$?
  [ "$rc" -eq 0 ] || { echo "::error::재발사 렌더 실패(rc=$rc)"; exit 1; }
  # 4) 새 이미지 → 카드N 자리 제자리 덮어쓰기 + versions 보존 + cards.md 텍스트 갱신
  ...
  commit_push "cards: $stem 카드$CARD_N 변경(재발사)"
  echo "::endgroup::"
  exit 0
fi
