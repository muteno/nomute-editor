...
- **2단계** (1단계 답 따라):
  - 포스트 → `이미지 합성`(분기②③) / `오버레이·문구만`(분기① — 기존 룰대로 릴스 OPA30 세트 동반)
  - 릴스 → `이미지 합성`(분기④) / `오버레이·강조형 *`(분기⑤) / `헤더형·강조 없음`(분기⑥ — "흰 배경 제거" 가능 안내)
  - 카피라이트·경고문 → `카피라이트`(F16) / `경고문`(F17)
- **확정 후** "문구 줘(합성이면 이미지 첨부)" 한 줄만 출력하고 대기. **문구가 오면 [의도 파악] 생략** — 트리 확정 분기 그대로 즉시 표준 흐름(모델 판단 = 맞춤법·강조 페어·오버플로/B안·합성 케이스·품질만). ⚠️ 헤더형 선택인데 문구에 `*`가 있으면 렌더 전에 한 줄로 지적하고 ⑤ 전환/`*` 제거를 되묻는다.
