...
for (const a of articles) {
  ...
  //  search.json = [{url, link}] (외부 hotlink, 복사 안 함 = Google CSE 결과)
  ...
  if (existsSync(tdir)) {
    try {
      ...
      if (Array.isArray(s)) thumbSearch = s.filter(x => x && x.url).map(x => ({ img: x.url, link: x.link || x.url }));
    } catch { /* 검색 없음 */ }
    try {
  ...
  a.cards = {
    ...
    thumb_search: thumbSearch,   // 검색 후보(P2 Google CSE) — 외부 hotlink
