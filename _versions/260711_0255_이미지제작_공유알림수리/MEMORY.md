...
> 📌 **뷰어 폼 UI(탭·goSpec·이미지 슬롯·수정복원·자간 안내) 작업이면 = `apps/thumbnail/01_지침_뷰어폼_v1.md`가 정본**(이 MEMORY·00은 렌더링/PIL 정본 · 폼은 거기 = 흔들림 방지 260624).
...
- 경고문(안내문 caution) 기본 2줄: `공익 목적의 사회 시사 콘텐츠입니다.` / `민감한 내용이 포함될 수 있으니 시청에 유의 바랍니다.` (정본=viewer/thumb.html WARN_PRESETS.caution · 운영자 260628)
...
**따옴표 자동 들여쓰기(F03b·260625)**: 여는 따옴표가 **줄 시작**에서 열려 다음 줄로 이어지면 연속줄을 *여는 따옴표 폭만큼* 자동 들여써 인용 첫 글자 아래로 정렬(`"6·25…`/`북침`의 `6`↔`북` 좌변 일치). **줄시작발만**(중간발 제외=좌측 낭비 방지·운영자 260625). 헬퍼 `quote_indent.py`의 `quote_indent_offsets(lines, fmt)`→lm_offsets(없으면 None)→`generate(..., lm_offsets=)`. 14종 따옴표+`*강조*`내부 추적·런타임 getbbox 진행폭·자간 sweep이 offset 합산. 폼(`thumb-make.yml` 앱1·2 오버레이) 자동. **`quote_indent.py`는 절대규칙1 4파일 아님**(별도 헬퍼·수정 가능).
...
- `quote_indent.py` — 따옴표 자동 들여쓰기(F03b). `quote_indent_offsets(lines,fmt,tracking=None)→lm_offsets|None`. **절대규칙1 4파일 아님**(별도 헬퍼). nomute_overlay의 SPECS/FONT_PATH/SCALE만 import.
