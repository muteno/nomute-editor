...
_DESIGN_BASELINE = {
    ...
    'viewer/ly.html': {'accent_raw': 0, 'blur': 14, 'hex': 16, 'accent_hex': 0, 'green_wash': 0},   # blur12→14·hex14→16 = 붙여넣기 폴백 모달(.pastefb) 신설 — backdrop blur(4px) webkit+표준 +2(ly엔 blur토큰 없어 raw) + 박스 배경 그라데이션 #14160f·#0c0f0c +2(기존 모달 배경 패턴·통일 기틀·운영자 260628)
    'viewer/k.html': {'accent_raw': 0, 'blur': 12, 'hex': 7, 'accent_hex': 0, 'green_wash': 0},
    'viewer/comp.html': {'accent_raw': 0, 'blur': 2, 'hex': 5, 'accent_hex': 0, 'green_wash': 0},   # STAGE1: --accent-rgb 추가·raw 7곳 토큰화 → accent_raw 7→0(픽셀0·k/ly 패턴·260628).
}
...
def check_design():
    ...
    for rel, base in _DESIGN_BASELINE.items():
        ...
        cnt = {'accent_raw': s.count('rgba(15,253,2'), 'blur': s.count('blur('),
               ...
               'green_wash': s.count('rgba(27,44,32') + s.count('rgba(24,40,29')}   # 초록 시그니처 워시(27,44,32=발행모달·dialog base main #1567 무채화 완료 + 24,40,29=크롬변종 .qflash·.failmenu·.dlgtop) = accent도 hex도 아닌 임의 rgba라 게이트 사각지대였음 → var(--modal-glass) 강제·차단(재검증5 완결성 봉합·분신술10 260704)
        for k, b in base.items():
            if cnt[k] > b:
                msg = '%s: raw %s %d > baseline %d → var() 토큰으로(§🎨)' % (rel, k, cnt[k], b)
                (hard if k in ('accent_raw', 'accent_hex', 'green_wash') else warns).append(msg)
    ...
    if hard:
        print('❌ 디자인 토큰 게이트(차단) — raw 강조색(rgba(15,253,2)·#0FFD02) 또는 초록 워시(rgba(27,44,32·24,40,29) 증가 = var(--accent)/var(--modal-glass) 토큰으로(요약본 템플릿 등 의도적 raw는 baseline 사유 기록 후 조정):')
        for w in hard:
            print('  -', w)
    if warns:
