...
SUBJ_PER_SEC = 2        # 초당 검출 횟수 — 3분 영상 +19s 실측(전신은 얼굴처럼 촘촘할 필요 없음 · 키잉 마스크는 SAM2가 매 프레임)
SUBJ_CONF = 0.40
SUBJ_NMS = 0.45
...
def main():
        ...
        if subj_net is not None and f % ystep == 0:
            ...
            for sx, sy, sw, sh, ssc, scls in sdets:
                ...
                if best >= SUBJ_IOU_MATCH and sbi >= 0:
                    ...
                    st = {"cls": scls, "kf": [box], "last_f": f, "votes": {}, "best_q": -1.0, "crop": None}
                    s_tracks.append(st)
                    s_active.append(st)
    ...
    out_people = []
    face_pid = {}      # id(얼굴트랙) → pid(캡 반영 후) — 피사체 연결 투표 해석용
