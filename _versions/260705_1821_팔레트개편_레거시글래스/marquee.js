...
const TILEBG  = '#242424';
const TILEED  = '#050505';
const RED     = '#d25563';        // letters on dark tile (손하트 발사색 — marqColor와 별개·불가침)
const CRIM    = '#c34551';        // letters on cream panel (손하트 발사색 — marqColor와 별개·불가침)
// 전광판 LOVE 글자색 = 설정(localStorage.marqColor) · pageAccent(reload/render)서 갱신 · 손하트(RED/CRIM)와 분리(운영자 260704)
let _MARQ_COL = '#c85c5c';
...
// ladder
const LAD = {topY:432, baseY:1078, topL:474, topR:576, baseL:320, baseR:730, rungs:5};   // baseY=캔버스 바닥 · rungs=5(간격 기준) · 레일은 아래로 LAD_EXT만큼 더 연장해 배너 아랫변에 잘림 = 맨 아랫변(바닥 크로스바) 없이 내려가는 열린 사다리(운영자 260704)
const LAD_EXT = 96;   // 레일을 캔버스 바닥(H) 너머 이만큼 더 뻗음 → 배너 아랫변이 잘라냄('자르고 아래로'=바닥 크로스바가 화면 밖으로 밀려 안 보임)
...
function drawLadder(lit){
  ...
  ctx.lineWidth=8;            // rails — 캔버스 바닥 너머(H+LAD_EXT)까지 같은 기울기로 연장 → 배너 아랫변이 잘라냄(둥근 stub 없이 깔끔히 잘림·운영자 260704)
  const extF=(H+LAD_EXT-LAD.topY)/(LAD.baseY-LAD.topY);
  const exL=lerp(LAD.topL,LAD.baseL,extF), exR=lerp(LAD.topR,LAD.baseR,extF);
  ctx.beginPath();
  ctx.moveTo(LAD.topL,LAD.topY); ctx.lineTo(exL,H+LAD_EXT);
  ctx.moveTo(LAD.topR,LAD.topY); ctx.lineTo(exR,H+LAD_EXT);
  ctx.stroke();
  ctx.lineWidth=6;            // rungs — 맨 아랫변(k=rungs=바닥 크로스바) 삭제: k<rungs (열린 사다리·운영자 260704)
...
function drawBoard(t, lit){
  ...
  for(let i=0;i<NTILE;i++){
    ...
    const _mc=_hexRgb(_MARQ_COL);   // 전광판 글자색(설정 marqColor) — 점등 시 살짝 어둡게(연출 보존)
    const col = rgb(lerpCol(_mc, [_mc[0]*0.82|0,_mc[1]*0.82|0,_mc[2]*0.82|0], lit));
