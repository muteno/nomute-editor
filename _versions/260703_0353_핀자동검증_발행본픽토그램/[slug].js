...
// 핀 입력 폼 — 뷰어 기틀 계승. 6자리 채우면 *자동 검증*(별도 '열기' 버튼 없음·운영자 260703).
// PIN 마스킹(현재 입력한 숫자만 노출·나머지 •) + MUT색 눈 토글(전체 표시). CSP 헤더 없음 → 인라인 style/script 동작.
// 흐름: 6자리 입력 → 320ms 후 자동 발사(중립 '확인 중' 글로우) → GET 네비게이션으로 서버 검증.
...
(function(){
  ...
  function arm(){clearTimeout(autoT);if(real.length===6&&!fired)autoT=setTimeout(fire,320);}   // 6자리 = 자동 검증(운영자 260703)
  function fire(){                                                // 자동 발사 = 중립 '확인 중' → 서버 검증(성공=문서 열림 / 실패=흔들림 재응답)
    ...
    setTimeout(function(){f.submit();},480);
  }
  ...
  eye.addEventListener('click',function(){
    ...
    render();if(!fired)pin.focus();
  });
  f.addEventListener('submit',function(e){e.preventDefault();fire();});   // Enter 등 = 자동발사 경유(6자리 미만이면 무시)
  // 서버가 틀린 핀으로 이 폼을 재응답(verr 메시지 존재) → 즉시 흔들림 + 빨강 점등(입력은 이미 빈 상태 = 처음부터)
