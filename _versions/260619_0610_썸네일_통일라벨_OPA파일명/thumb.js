...
export async function onRequestPost({ request, env }) {
  ...
  if (app === '4' || app === '1') {           // 경고문 / 포스트 — lines(강조 *...* 허용)
    ...
    if (app === '1') {
      ...
      // opas[] = 다중 선택(투명도 토글) · 하위호환 단일 opacity 도 통과
      if (Array.isArray(p.opas) && p.opas.length) {
        const opas = p.opas.filter(o => Number.isFinite(+o) && o !== '').map(o => Math.max(0, Math.min(100, Math.trunc(+o))));
    ...
    if (mode === 'header') {
      ...
      const lines = cleanLines(p.lines);
      if (!lines.length) return json({ error: '텍스트 줄(lines)이 필요해' }, 400);
      // 선택된 opa(60·30·직접입력 멀티·최소1) — 프론트가 점등분만 보냄. 정리(0~100·정수·중복제거).
      let opas = [...new Set((Array.isArray(p.opas) ? p.opas : [])
        .map(n => Math.trunc(+n)).filter(n => Number.isFinite(n)).map(n => Math.max(0, Math.min(100, n))))];
