...
| SNS 소스칩 | `.soc-chip`(base 시안)·`.lean-l/-r`(정치성향)·`.src-naver`+`-cafe/-blog/-kin`(네이버) | `--fs`10.5·pill | base `rgba(--info-rgb,.1)` / 네이버 `--naver`(#03C75A·accent와 hue 30°이격·매트) 카페`.18`/블로그`.08`/kin 점선 · lean 파/빨 | — | — | `.soc-srcs` 카드 하단 | `2×8` | 텍스트=소스명(색·형태 보강) | index `renderSns`·`socLean`/`socSrc`·`SOC_SRC_MAP` (네이버 통합 260704) |
