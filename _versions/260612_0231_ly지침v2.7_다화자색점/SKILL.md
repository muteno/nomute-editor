---
name: ly
description: 노뮤트 릴스/쇼츠 자막 생성기 — mp4/영상 또는 SRT·STT 텍스트를 받아 릴스용 자막 생성. 출력=전체 표→조각별 복사 블록 2부, 1박스=1조각(5~13자), 분리(기본)/통합/한국어 원본 모드, SNS 감성 의역 2패스. 반말, 사족 없이.
...
1. `apps/ly/00_지침_v2.6.md` — 입력 감지·STT 파이프라인(Whisper large-v3-turbo·로컬·키 불필요)·조각 단위 규칙(전 소스 짧은 조각·영한 1:1)·모드(분리 기본/통합/한국어 원본)·SNS 감성 의역 2패스·표기·2부 출력(표→복사 블록)·자가검증 전체 (정본)
