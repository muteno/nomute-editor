...
CAT_KW = {
    "국제": ["트럼프", "바이든", "미국", "중국", "일본", "러시아", "우크라", "이란", "이스라엘", "가자", "전쟁", "외교", "정상", "종전", "백악관", "관세", "푸틴", "시진핑", "나토", "중동", "북한", "김정은", "파병"],
    "경제": ["경제", "금융", "증시", "주가", "코스피", "코스닥", "환율", "부동산", "집값", "금리", "매출", "영업", "실적", "투자", "수출", "무역", "물가", "부채", "고용", "세금", "상장", "은행", "인수", "합병", "스타트업"],
    "문화": ["영화", "드라마", "음악", "가수", "배우", "공연", "전시", "예술", "연예", "아이돌", "스포츠", "축구", "야구", "올림픽", "월드컵", "중계", "게임", "웹툰", "콘서트", "앨범", "예능", "감독", "넷플릭스", "OTT", "시즌", "데뷔", "컴백", "신곡", "개봉", "관객", "흥행", "박스오피스", "빌보드", "그래미", "뮤지컬", "박물관", "미술관", "축제", "만화", "문학", "소설", "작가", "OST", "방탄", "BTS", "블랙핑크", "뉴진스", "아이브", "세븐틴", "에스파", "르세라핌", "MLB", "KBO", "프로야구", "홈런", "손흥민", "이정후", "류현진", "메달", "우승", "결승", "갤러리", "패션",
    ...
    "사회": ["사건", "사고", "경찰", "검찰", "법원", "재판", "판결", "구속", "파업", "교육", "학교", "복지", "의료", "병원", "범죄", "화재", "시위", "노조", "수사", "고소", "갑질", "마약",
            "음주운전", "기소", "입건", "혐의", "체포", "송치", "압수수색", "구형", "사망", "성범죄", "폭행"],   # 범죄·사법 절차어 보강(운영자 260627·평의회5 — 셀럽 범죄가 문화로 안 새게 사회-우선 tie-break 실효화)
}


# 하트(♥♡❤ 등) — 제목에 들어가면 연예인 열애/결혼·2세 = 100% 문화(운영자 260624). 사회 오분류·미분류 구제.
HEART_RE = re.compile("[♡♥❣❤\U0001F493-\U0001F49F\U0001F9E1\U0001FA77]")
# OSEN(오!쎈) — 조선일보 문화부 스포츠·연예 브랜드 → 100% 문화(운영자 260625). 제목 [오!쎈]/[OSEN=] 태그로 잡힘.
...
def cat_of(category, title, media=""):
    ...
    for k, ws in CAT_KW.items():
        ...
        if n > bn or (n == bn and n > 0 and k == "사회"):   # 사회-우선 tie-break(운영자 260627·평의회5) — 셀럽 범죄("열애설 OOO 폭행 고소")는 문화·사회 동점이면 사회 보존
            bn, best = n, k
    if best == "문화" and CRIME_OVERRIDE_RE.search(t):   # 셀럽이라도 범죄·사법 절차면 사회(평5 하드가드 — 문화어 다수여도 범죄 우선)
        return "사회"
