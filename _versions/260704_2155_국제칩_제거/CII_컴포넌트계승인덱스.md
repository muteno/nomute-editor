...
| 단독 필터 토글 | `.sc-chips .sc-tg`(수집함 국제 칩 `.sc-intl` — `.sc-tg` 패널 버튼 패밀리 계승) | h24(.sc-tg base)·pad `0 12px`·fw700(on=800) | `.sc-tg` 글래스 필(`rgba(8,15,11,.34)`+border `.14`+blur8) · on=`.sc-tg.ack.on` 값 계승 `rgba(var(--accent-rgb),.14)`+border `.5`+`--accent` 글자(상태색=무채 룰 예외) | `--press-m`(.sc-tg 상속) | 텍스트만 | 패널 우상단(`.sc-chips` flex-end·그리드 전폭) | `0 0 2px` | `aria-pressed` | **⛔ 단독(형제 없는) 토글에 텍스트-온리 `.chip` 금지** — `.chip`은 칩 바(형제 여럿) 전제·단독 배치 시 고아 라벨로 읽힘(260704 실측·§🎨 단독 토글 룰). 계승 소스 = 생김새 아닌 *배치 맥락·표면*(수집함=`.sc-tg`·피드=`.sbtn`) |
