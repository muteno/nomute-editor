...
BARE = os.environ.get("GATE_BARE", "0").strip().lower() not in ("0", "false", "no", "")   # --bare 기본 ON: CLAUDE.md auto-discovery·hooks·skills·MCP·auto-memory 스킵 = 분류에 안 쓰이는 라우터 99KB(~40k토큰) 컨텍스트 제거 → cache_w ~95%↓(260701). RUBRIC은 stdin이라 판정 무영향. 롤백=env GATE_BARE=0.
