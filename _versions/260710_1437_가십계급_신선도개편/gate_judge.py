...
⚠️ 연예 예외 — **최정상급(국민 대부분이 아는 이름) 연예인·유명인의 열애·결별·결혼·이혼·사망·중대사고의 공식 확인·단독 보도**는 가십이 아니라 전국민적 관심 사건 = **[1]**([2]·[3] 금지 — 긴급·주목 아님, 목록에 뜨는 경미 뉴스로만). 무명·중소 연예인의 동종 소식, 그리고 최정상급이어도 화보·근황·목격담·일상·패션·열애설(미확인 지라시)은 여전히 [0]. 예: '아이유·이종석, 4년 열애 끝 결별'(소속사 확인) = [1] / '배우 A씨 열애설 포착' = [0] / '아이유 공항패션 화제' = [0].
