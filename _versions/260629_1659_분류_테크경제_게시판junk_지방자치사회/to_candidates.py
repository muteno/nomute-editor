...
# 정치 인물·강마커 = 정치 강제(AI 분류 *이후* 추가 키워드 · 운영자 260629). 李·尹 헤드라인 약물 + 정치인 실명이 CAT_KW에 없고 사회-우선 tie-break이 정치를 덮어 사회 catch-all로 새던 것 교정.
# ⚠️ 보수 스코프 = **사회/미분류만 → 정치**(경제·국제·문화·테크 주제는 유지 → 中 李克強=국제·정치인 경제정책=경제 오분류 방지). 인물셋은 시류 따라 운영자 튜닝(stale 가능).
# 인물 = 현 주요 정치인 실명 + 헤드라인 한자약물(李=이재명·尹=윤석열) / 마커 = 불명확한 일반역할(장관·의원·정부) 제외, 강마커(대통령·탄핵·내란·여야·정당·선거·당직)만.
POL_FORCE_RE = re.compile(r"李|尹|이재명|윤석열|한동훈|이준석|조국|김건희|김문수|우원식|한덕수|추미애|홍준표|안철수|오세훈|이낙연|나경원|김기현|권성동|정청래|박찬대|대통령|대통령실|청와대|탄핵|내란|여당|야당|국민의힘|민주당|대선|총선|공천|비대위|당대표|원내대표|거부권|국정감사|영부인")
