...
from track_keying import (KEY_BUDGET_SEC, KEY_MAX_LONG, KEY_MAX_OBJ, KEY_MAX_SEC, IMGSZ,
                          ...
                          SAM_CKPT, _num, plan_passes)   # 전파 계획·캡 = 키잉 단일 출처(드리프트 0)
...
def run(src, tracks, req, out_path):
    ...
    W2, H2 = W - (W % 2), H - (H % 2)
    total_f = float((doc.get("meta") or {}).get("frames") or 0) or (cap.get(cv2.CAP_PROP_FRAME_COUNT) or 0)
    real_dur = total_f / fps if total_f else float((doc.get("meta") or {}).get("dur") or 0)
