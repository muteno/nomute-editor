...
function Install-Startup {
    ...
    $lines = @(
        ...
        ('start "" /min powershell.exe -NoProfile -WindowStyle Hidden -ExecutionPolicy Bypass -File "' + $agent + '"')
    )
...
try {
    ...
    if (-not (Test-Path -LiteralPath $Source)) { Write-Log "원본 폴더가 없다: $Source" 'FAIL'; return }
