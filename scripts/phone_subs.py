...
_xkw = [t.get("query") for t in st.x_trends(limit=6) if t.get("query")]   # ⑯ X 검색 시드 = X 실시간 트렌드 키워드(무인증 trends24 · 홈IP 정상) 상위 6 → x_search 인증검색
out = {"x": st.x_subs(acc["x"], limit=20), "insta": st.insta_subs(acc["insta"], limit=20),
       "xsearch": st.x_search(_xkw, per=8, limit=15) if _xkw else [],   # ⑯ X 검색 인기글(운영자 260723 · env X_AUTH_TOKEN+X_CT0 가계정 전용 · 미설정 = [] no-op) — 트렌드 키워드로 X 검색
...
print(f"phone-subs 수집: x {len(out['x'])}건 · xsearch {len(out['xsearch'])}건 · insta {len(out['insta'])}건 · threads {len(out['threads'])}건 · tiktok {len(out['tiktok'])}건 · reddit {len(out['reddit'])}건 · 재난 {len(out['disaster'])}건")
