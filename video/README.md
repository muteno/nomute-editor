...
지금은 **`queue/`의 파일명 역순(= 최신순)에서 위에서부터 N건**이다. 골라내는 기능은 없다.
특정 기사만 뽑고 싶으면 지금은 `.build/rows.json`을 열어 필요 없는 행을 지우고 배치를 돌리면 된다.
(※ `.build/`는 기계산출물이라 다음 `build.mjs` 실행에 덮어써진다 — 상시로 쓸 거면 `build.mjs`에
   "파일명 지정" 옵션을 넣는 게 정석이다. **보완 후보 1**)
...
1. **기사 선택** — 지금은 최신 N건 자동. 파일명·날짜 지정 옵션 필요.
