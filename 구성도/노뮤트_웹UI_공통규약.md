...
- **수집함 배지** 긴급(빨강)/이슈(노랑) = **메타라인 인라인 텍스트**(`카테고리 · 매체 N · 배지` 한 줄, 좌측 메타군 + 상대시간 우측 · `.sc-badge`). 글래스 칩·절대배치(`.tagrow`) 폐지(운영자 260621). 강조색 = 매체 N(라임)·이슈(노랑)·긴급(빨강) 셋만 · 카테고리·구분점·시간 = 뮤트/흰. 폰트=Pretendard·자간0(Orbitron 미사용). 점등(테두리 b-brk/b-iss·글로우 lit-*)은 별개 조건 유지.
