...
**토큰 밖 매직 색 → 토큰화**: 인라인 매직 색은 가장 가까운 토큰으로. ⚠️ 260704 개편 정정: `--fg-2` 폐지(→`--mut`) · `--accent-2` = **보라 #c24bf5**(이력/최소화 · 옛 "앰버 #ff9614" 설명 폐기) · `--accent-bright` = `--accent-4`(라임 #d8ff3d) · 의미색(`--danger/--warn/--amber/--info/--arm`) = accent-3~5 별칭.
