...
새 색 지어내지 않음. 메뉴 = 그 메뉴의 배너 글로우색. **hex 실체는 이 표에 안 적는다(260721 Q345 — 표가 :root보다 낡는 사고 재발 차단 · 값 = :root 정본)**.

| 메뉴 | 대표색 | 토큰 |
|---|---|---|
| 피드·큐레이션 | 터쿼이즈 | `--accent` |
| 스크랩 | 골드레몬 | `--accent-4` |
| SNS | (정본 = `:root` 실사용 글로우 — 이 칸의 구 hex·구 토큰 표기가 260705 스왑을 놓친 실증 드리프트) | `:root` 참조 |
| 썸네일 생성기 | 골드레몬 | `--thumb`(accent-4 · 강조색 터쿼이즈와 구분) |
| ly·k | 라임(기본) | 별도 색 없음 |
